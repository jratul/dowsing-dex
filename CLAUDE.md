...
>   뭉개진다(니로우가 그랬다).
> 출현 페이지의 **버전 필터는 위키 라벨이 아니라 게임 단위**다(`src/lib/encounterVersions.ts`).
> 위키는 두 게임 공통이면 「하트골드·소울실버」, 한쪽 전용이면 「하트골드」로 적는데, 라벨을
> 그대로 필터로 쓰면 셋이 따로 놀아 **하트골드 전용 33종이 안 보인다.** 그래서 라벨을 `·` 로
> 쪼개 게임 목록으로 풀고, 게임을 고르면 그 게임을 포함하는 라벨을 모두 보여 준다.
> **4세대 라벨은 한국 정발명으로 모은다** — 다이아몬드→디아루가, 펄→펄기아,
> 플라티나/플래티넘→기라티나. 한국닌텐도 정식판이 「포켓몬스터DP 디아루가·펄기아」·
> 「포켓몬스터Pt 기라티나」이고, 「다이아몬드·펄」은 대원씨아이가 들여온 **일본어판 수입**
> 명칭이다(WORD.md 「신오지방」 참고). 그래서 **「전용」 표시는 라벨이 게임 하나만 가리킨다고
> 붙이면 안 된다**: 기라티나는 짝 라벨이 없어 혼자 나올 뿐 전용이 아니다. 짝 라벨(`A·B`)로도 등장하는 게임에서만 단독 라벨을
> 전용으로 본다.
...
>
> `scripts/check-guide-movetables.mjs`는 `.data.ts` 의 `moveTable` 행과 `finalMoves`
> 배열을 학습셋과 대조한다. **`machines` 검사기는 본문 문장만 본다** — 기술 배치표는
> 기술명(`move`)과 습득 방법(`how`)이 서로 다른 필드에 있어 둘이 어긋나도 정규식에
> 안 걸린다. 화면에는 기술 이름과 초록 뱃지가 멀쩡히 보여 눈으로는 절대 못 잡는다.
> 잡아내는 것: 실존하지 않는 기술명, TM/HM 번호↔기술 불일치, 습득 레벨·방법 불일치,
> 그 게임에서 못 배우는 기술, `finalMoves` 중복, 그리고 `usage` 설명의 타입·물리특수
> 분류가 실제와 다른 경우. **1~3세대는 기술이 아니라 타입으로 물리/특수가 갈리므로**
> (3세대 고스트=물리, 드래곤=특수) 그 세대 공략은 타입 기준으로 판정한다.
> 진화 전 형태가 배우는 기술(무우마 Lv.50 파워젬 → 무우마직)은 정상으로 본다.
>
> ```bash
...
> ```
>
> `scripts/check-guide-vocab.mjs`는 **[WORD.md](./WORD.md) 의 「잘못된 표기 → 올바른 표기」
> 표를 그대로 사전으로 삼아** 원문을 훑는다. 지역명·NPC명·배지명은 대조할 생성 데이터가
> 없어 그동안 아무 검사도 못 받았다. 사람이 발견해 WORD.md 에 적어 둔 오표기를 코드가
> 다시 읽게 하면 같은 실수가 새 글에 들어왔을 때 바로 잡힌다. 두 가지를 조심한다 —
> **표기는 지역마다 다르고**(「물가시티」는 호연에서는 보라시티의 오표기지만 신오에서는
> Sunyshore City 의 정식 명칭이다) 표가 놓인 `##` 절을 보고 해당 세대에만 적용하며,
> **오표기가 실존 이름이거나 그 일부인 경우**(「뚜꾸리」는 #498 Tepig 의 정식 이름,
> 「우츠보」는 우츠보트의 일부)는 건너뛴다.
>
> ```bash
> node scripts/check-guide-vocab.mjs
> ```
>
> 공략을 추가·수정하면 위 일곱 스크립트(`pokemon-ids`·`machines`·`levels`·`roster`·
> `items`·`movetables`·`vocab`)를 모두 돌린다. 2026-08 전수 점검에서 이 검사들이 잡아낸 것:
> 파이어레드 돌 진화 표의 1세대 레벨, 에메랄드 공략의 세대 초과 포켓몬(삼삼드래·
> 불비달마·미끄래곤), 하트골드 스토리 공략의 관동 체육관 명단 전체, 플래티넘 공략의
> 비공식 기술명 39건. `movetables` 를 뒤늦게 추가하자 곧바로 43건이 더 나왔다 —
> 플래티넘 HM02 를 제비반환이라 적어 둔 것(4곳), 에메랄드 HM08 다이빙을 패리퍼에게
> 배정한 것(패리퍼는 못 배운다), 없는 기술명(야이바스터·혹독한빛·염원구슬·밤의소란).

> **저장소 밖의 원문 노트도 같은 검사를 받을 수 있다.** 옵시디언 볼트처럼 공략의
> 원본이 되는 마크다운이 따로 있으면 아래처럼 돌린다. 파일별 게임은
> `guide-sources.mjs` 의 `EXTERNAL_GAMES` 에 적고, 표에 없는 `.md` 가 나오면 에러를
> 내므로 조용히 빠지지 않는다.
>
> ```bash
> GUIDE_EXTERNAL_DIR="C:/Users/yky/Documents/Obsidian Vault/포켓몬" >   GUIDE_EXTERNAL_ONLY=1 node scripts/check-guide-vocab.mjs
> ```
>
> 2026-08 볼트 점검에서 잡은 것: 칸토/조토(4건), 얼다바람(11건), 깨트리다(10건),
> 번개의돌(9건), 스타미(11건), 비상배지(4건), 별가사리 HP회복 레벨을 1세대 값으로
> 적은 것(3건 — 파이어레드는 Lv.15 다).
...
| `/encounter` | EncounterPage | 게임별 야생 출현 및 포획 불가 포켓몬 + 방법별 출현 확률·시간대·버전 전용 |
...
| `/guides/pokemon-gold-silver-party` | PokemonGoldSilverPartyGuidePage | 정적 전용 라우트 (2세대 금·은 파티·HM 배치) |
...
- **진화 조건도 세대마다 다르다**(`EvolutionStage.triggerByGeneration`, `triggerForGeneration()`).
  리피아는 4~7세대 이끼바위 근처 레벨업, 8세대부터 리프의돌이다. PokeAPI 는 한 진화에 **게임별
  조건을 여러 개** 주는데 `fetch-pokedex.mjs`가 예전엔 첫 항목만 쓰고 장소(`location`·
  `near_special_rock`)와 걸음 수(`min_steps`)를 못 읽어 9종이 그냥 "레벨업"으로 남았다.
  지금은 세대별로 묶되 두 가지를 조심한다 — **레전드 아르세우스 조건은 본편 조건이 있으면 뺀다**
  (마그케인 Lv.17 이 8·9세대 전체로 퍼진다), **교환 진화는 한번 생기면 이후 세대에도 남긴다**
  (PokeAPI 의 밀로틱 ORAS 항목엔 아름다움 진화만 있어 고운비늘 교환이 사라진다).
  `trigger`는 PokeAPI `is_default` 조건(최신)이라 세대를 모르는 아이템 페이지가 그대로 쓴다.
- **기술표도 당시 값이다**(`lib/pastMoves.ts`의 `moveForVersion`). 페이지가 `findMove`를 감싼
  `findMoveForTab`을 기술 컴포넌트에 넘긴다. 이력은 `scripts/build-past-move-values.mjs`가 PokeAPI
  `move_changelog.csv`에서 뽑는다(직접 실행). 행은 "그 버전 그룹에서 바뀌기 **직전** 값"이고, 한 세대
  안에서 바뀐 기술이 있어(최면술 명중 DP 70 → Pt 60) **세대가 아니라 버전 그룹 순서로 비교**한다.
  레츠고 행은 직전 값이 현재 값과 같아(레츠고에서만 바뀌고 소드·실드에서 돌아왔다) 제외한다.
  1~3세대는 물리·특수를 타입으로 다시 정한다(2세대 물기 = 악 = 특수).
