...
export const HGW_WEEK: WeeklyDay[] = [
  {
    ...
    tasks: [
      ...
      { kind: 'phone', title: '관장 전화번호 — 비상 · 도희 · 유빈', place: '무지개시티 백화점 4층 / 인주시티 방울탑 가는 길', detail: '★ 월요일 백화점 4층에서 비상과 도희가 서로 아버지가 더 강하다며 말다툼한다. 여기서 한쪽 편을 들면 그 사람의 번호만 받고 두 사람 모두 그 자리에서 사라진다 — 그 주에는 나머지 한 명의 번호를 받을 수 없다. 유빈은 월·화요일에 방울탑 가는 길에 있다.' },
    ],
  },
  {
    day: '화',
    headline: '곤충채집 대회가 열리는 첫 날',
    tasks: [
      { kind: 'sibling', title: '화홍에게 휘어진스푼', place: '29번도로', detail: '2세대에서는 핑크리본을 줬지만 HGSS 에서는 휘어진스푼으로 바뀌었다. 리본은 철렁리본.' },
      {
        kind: 'contest', title: '자연공원 곤충채집 대회', place: '자연공원 (금빛시티 북쪽)', priority: true,
        detail: '화·목·토요일에만 열린다. ★ 20시에 가면 「오늘 대회는 끝났다」며 돌려보내니 그 전에 접수한다. 참가비는 없고 하루 한 번. 접수할 때 선두에 둔 포켓몬 한 마리로 20분 동안 스포츠볼 20개를 써서 벌레 한 마리를 잡아 겨룬다. 1등 상품은 전국도감 전이면 태양의돌, 얻은 뒤에는 진화용 돌 8종 중 랜덤 1개다(2등 변함없는돌, 3등 자뭉열매, 참가상 아름다운허물). 스라크·쁘사이저처럼 평소에 안 나오는 벌레도 나온다.',
      },
  ...
  },
  {
    ...
    tasks: [
      ...
      { kind: 'contest', title: '자연공원 곤충채집 대회', place: '자연공원', detail: '화·목·토 중 둘째 날. 전국도감을 얻은 뒤에는 목요일에만 호연지방 포켓몬이 섞여 나온다.' },
  ...
  {
    day: '토',
    headline: '곤충채집 대회 마지막 날 + 한약방',
    tasks: [
      { kind: 'sibling', title: '토영에게 부드러운모래', place: '검은먹시티', detail: '2세대에서는 저주의부적이었지만 HGSS 에서는 부드러운모래로 바뀌었다. 리본은 잠보리본.' },
      { kind: 'contest', title: '자연공원 곤충채집 대회', place: '자연공원', detail: '한 주의 마지막 기회. 놓치면 화요일까지 기다린다. 전국도감을 얻은 뒤에는 토요일에만 신오지방 포켓몬이 섞여 나온다.' },
      ...
      { kind: 'phone', title: '관장 전화번호 — 민화', place: '무지개시티 분수 근처', detail: '토·일요일 낮 3시~5시에만 나와 있다. 토요일에 받아 두면 다음 날 아침 재대결로 바로 이어진다.' },
    ],
  },
  {
    day: '일',
    ...
    tasks: [
      ...
      { kind: 'phone', title: '관장 전화번호 — 민화', place: '무지개시티 분수 근처', detail: '토·일요일 낮 3시~5시에만 나와 있다. 시간대가 두 시간뿐이라 놓치기 쉽다. 재대결은 일요일 아침이다.' },
    ],
  },
]
...
export const HGW_PRIORITY: WeeklyPriority[] = [
  ...
  { rank: 4, day: '화 · 목 · 토', what: '곤충채집 대회', why: '1등 상품이 진화의 돌. 주 3회라 여유가 있어 보이지만 대회 시간대가 정해져 있어 자주 놓친다.' },
...
export const HGW_REMATCH_RULES: { title: string; body: string }[] = [
  ...
  {
    title: '비상·도희는 한 번의 월요일에 한 명만 받는다',
    body: '★ 실제 플레이로 확인한 함정이다. 무지개시티 백화점 4층의 말다툼에서 한쪽 편을 들면 그 사람의 번호만 등록되고, 두 사람이 동시에 그 자리에서 사라진다. 「둘 다 받았겠지」 하고 넘어가면 나머지 한 명의 재대결(비상은 토요일 아침, 도희는 월요일 낮)이 영영 안 열린다. 번호를 받은 뒤 포켓기어 전화번호부에서 두 사람이 다 있는지 확인한다.',
  },
  {
