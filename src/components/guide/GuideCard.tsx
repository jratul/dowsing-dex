...
export function GuideCard({ guide, iconSpriteUrl, variant = 'banner' }: GuideCardProps) {
  ...
  if (variant === 'row') {
    return (
      <Card className="flex gap-3 p-3">
        <div className={cn('relative flex h-20 w-20 shrink-0 items-center justify-center rounded-button', style.bannerClass)}>
          <span className={cn('absolute top-1 left-1 rounded-chip bg-white px-2 py-0.5 text-[10px] font-bold', style.pillClass)}>
            {guide.category}
          </span>
          {iconSpriteUrl && <img src={iconSpriteUrl} alt="" width={48} height={48} />}
        </div>
  ...
  return (
    <Card className="overflow-hidden p-0">
      <div className={cn('relative flex h-28 items-center justify-end px-4', style.bannerClass)}>
        <span className={cn('absolute top-3 left-3 rounded-chip bg-white px-2.5 py-1 text-xs font-bold', style.pillClass)}>
          {guide.category}
        </span>
        {iconSpriteUrl && <img src={iconSpriteUrl} alt="" width={72} height={72} />}
      </div>
