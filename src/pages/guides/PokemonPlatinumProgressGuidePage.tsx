...
const GYM_PREREQS = [
  ...
  },
  {
    ...
    tip: '선단시티 자체까지는 HM08 록클라임가 없어도 도달 가능. 록클라임는 7관 배지 후 사용',
  },
  {
    gym: '8관 물가시티 — 전진 (전기)',
    steps: [
      { label: '글레이셔배지(7관) 취득', detail: 'HM08 록클라임 필드 사용 조건' },
      { label: '갤럭시단 최종 이벤트 완료', detail: '천관산 → 스피어필라 → 깨어진세계(기라티나 이벤트). 이 이벤트를 완료해야 물가시티에 입장 가능' },
      { label: 'HM08 록클라임 + 210번도로 절벽', detail: '210번도로 동쪽 절벽을 록클라임로 내려가면 물가시티 방향 진입 경로 확보' },
  ...
  },
  {
    ...
    steps: [
      ...
      { label: '승리로드 통과', detail: '풀베기(HM01)·바위깨기(HM06)·괴력(HM04)·록클라임(HM08) 등 다수 필요. 폭포오르기(HM07)로 내부 폭포 등반 필수' },
    ],
...
const AREA_REQS = [
  ...
  {
    area: '물가시티',
    requirement: '갤럭시단 최종 이벤트(깨어진세계) 완료 + HM08 록클라임 + 7관 글레이셔배지',
    detail: '깨어진세계 이벤트 완료 전까지 물가시티로 들어가는 길 자체가 막힌다. 완료 후 210번도로 동쪽 절벽을 록클라임로 내려가 진입',
  },
  {
    area: '승리로드',
    requirement: 'HM01·06·04·08·07 다수 필요',
    detail: '풀베기·바위깨기·괴력·록클라임·폭포오르기를 전부 사용하는 구간이 있다. 파티에 HM 담당 포켓몬 배치 필수',
  },
]
      ...
      <Card className="mb-6 p-4">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-ink-muted">천관산 → 스피어필라 → 깨어진세계(기라티나 이벤트)를 완료해야 물가시티 진입 경로가 열린다. 7관을 깬 직후에도 이 이벤트를 완료하기 전에는 갈 수 없다. 210번도로 동쪽 절벽을 HM08 록클라임로 내려가는 경로가 열린다.</p>
          </div>
          <div>
