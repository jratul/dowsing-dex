...
const GYM_PREREQS = [
  ...
  },
  {
    gym: '2관 영원시티 — 유채 (풀)',
    steps: [
      { label: '콜배지(1관) 취득', detail: 'HM01 풀베기 필드 사용 조건. 1관을 먼저 깨야 한다' },
  ...
  },
  {
    ...
    steps: [
      { label: '포리스트배지(2관) 취득', detail: 'HM05 안개제거 필드 사용 조건' },
  ...
  },
  {
    ...
    steps: [
      { label: '코블배지(3관) 취득', detail: 'HM02 공중날기 필드 사용 조건' },
  ...
  },
  {
    ...
    steps: [
      { label: '펜배지(4관) 취득', detail: 'HM06 바위깨기 필드 사용 조건. 멜리사는 4관 취득 후 연고시티로 돌아온다' },
  ...
  },
  {
    gym: '6관 운하시티 — 동관 (강철)',
    steps: [
      { label: '레릭배지(5관) 취득', detail: 'HM03 파도타기 필드 사용 조건' },
  ...
  },
  {
    gym: '7관 선단시티 — 무청 (얼음)',
    steps: [
      { label: '마인배지(6관) 취득', detail: 'HM04 괴력 필드 사용 조건' },
    ...
    tip: '선단시티 자체까지는 HM08 락클라임가 없어도 도달 가능. 락클라임는 7관 배지 후 사용',
  },
  {
    gym: '8관 물가시티 — 전진 (전기)',
    steps: [
      { label: '글레이셔배지(7관) 취득', detail: 'HM08 락클라임 필드 사용 조건' },
      { label: '갤럭시단 최종 이벤트 완료', detail: '천관산 → 스피어필라 → 깨어진세계(기라티나 이벤트). 이 이벤트를 완료해야 물가시티에 입장 가능' },
      { label: 'HM08 락클라임 + 210번도로 절벽', detail: '210번도로 동쪽 절벽을 락클라임로 내려가면 물가시티 방향 진입 경로 확보' },
      { label: '전진 이벤트', detail: '물가시티 체육관에 전진이 없다. 등대에 있는 전진을 찾아가 대화 → 체육관으로 돌아옴 → 8관 도전 가능' },
    ],
    tip: '★깨어진세계 이벤트를 완료하지 않으면 물가시티 자체에 진입할 수 없다. 7관 후 바로 가려 해도 막힘',
  },
  {
    gym: '사천왕 도전 (승리로드)',
    steps: [
      { label: '비컨배지(8관) 취득', detail: 'HM07 폭포오르기 필드 사용 조건' },
      { label: 'HM07 폭포오르기 입수', detail: '물가시티 NPC에게서 수령' },
      { label: '승리로드 통과', detail: '풀베기(HM01)·바위깨기(HM06)·괴력(HM04)·락클라임(HM08) 등 다수 필요. 폭포오르기(HM07)로 내부 폭포 등반 필수' },
    ],
...
const GALACTIC_EVENTS = [
  ...
  { step: 7, location: '운하시티 강철섬', event: 'Buck와 함께 강철섬 탐색 → 갤럭시단 격파 → Buck가 강철섬 탈출. 루카리오 알 이벤트' },
  { step: 8, location: '장막시티 갤럭시 본부 (재방문)', event: '갤럭시 열쇠 입수 → 본부 내부 탐색 → 간부 격파 → 포켓몬 해방' },
  { step: 9, location: '천관산 → 스피어필라', event: '기아루가·알기아를 소환하는 수장 키쿠이 저지. 기라티나가 개입 → 깨어진세계로 이동' },
  { step: 10, location: '깨어진세계 (기라티나 영역)', event: '깨어진세계를 탐색해 기라티나와 조우. 포획 or 격파 후 현실로 귀환. 물가시티 진입 개방' },
]
...
const AREA_REQS = [
  ...
  },
  {
    ...
    requirement: 'HM05 안개제거 + 2관 포리스트배지',
    detail: '신수마을 ~ 봉신마을 사이 안개 구간. 안개제거를 쓰지 않으면 시야가 좁아 이동이 불가에 가깝다',
  },
  {
    area: '212번도로 남부 진흙길',
  ...
  {
    area: '운하시티',
    requirement: 'HM03 파도타기 + 5관 레릭배지',
    detail: '218번도로 전체가 수중. 파도타기 없이는 축복시티 서쪽으로 나갈 수 없어 운하시티 진입 불가',
  },
  {
    area: '천관산 상층부',
    requirement: 'HM06 바위깨기 + 4관 펜배지, HM04 괴력 + 6관 마인배지',
  ...
  {
    area: '물가시티',
    requirement: '갤럭시단 최종 이벤트(깨어진세계) 완료 + HM08 락클라임 + 7관 글레이셔배지',
    detail: '깨어진세계 이벤트 완료 전까지 물가시티로 들어가는 길 자체가 막힌다. 완료 후 210번도로 동쪽 절벽을 락클라임로 내려가 진입',
  },
  {
    area: '승리로드',
    requirement: 'HM01·06·04·08·07 다수 필요',
    detail: '풀베기·바위깨기·괴력·락클라임·폭포오르기를 전부 사용하는 구간이 있다. 파티에 HM 담당 포켓몬 배치 필수',
  },
]
      ...
      <Card className="mb-6 p-4">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-ink-muted">천관산 → 스피어필라 → 깨어진세계(기라티나 이벤트)를 완료해야 물가시티 진입 경로가 열린다. 7관을 깬 직후에도 이 이벤트를 완료하기 전에는 갈 수 없다. 210번도로 동쪽 절벽을 HM08 락클라임로 내려가는 경로가 열린다.</p>
          </div>
          <div>
            <SubHeading>Q. 운하시티(6관)로 가려면 어떻게 하나</SubHeading>
            <p className="text-sm text-ink-muted">축복시티 서쪽의 218번도로가 수중 구간이라 파도타기가 필수다. 파도타기는 봉신마을 갤럭시단 이벤트를 해결한 후 박사 할머니에게서 받는다. 5관 레릭배지가 있어야 필드에서 사용 가능.</p>
          </div>
          <div>
            <SubHeading>Q. 봉신마을(파도타기 입수)까지 어떻게 가나</SubHeading>
            <p className="text-sm text-ink-muted">연고시티 → 동쪽 209번도로 → 신수마을 → 210번도로 북쪽 → 봉신마을. 210번도로 북부는 안개 구간이므로 HM05 안개제거 + 2관 포리스트배지가 있어야 편하게 통과할 수 있다.</p>
          </div>
          <div>
