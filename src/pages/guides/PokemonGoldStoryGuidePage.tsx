    ...
    <GuidePageLayout generation="2세대" version="골드·실버">
      ...
      <Card className="mb-6 p-4">
        ...
        <GuideTable
          ...
          rows={GOLD_STORY_HM_TABLE.map((r) => [r.hm, <MoveLink key={`${r.hm}-mv`} name={r.move} stats />, L(r.pokemon), r.location, r.badge, r.note])}
        />
      </Card>
      ...
      <Card className="mb-6 p-4">
        ...
        <GuideTable
          ...
          rows={GOLD_STORY_TM_PRIORITY.map((r) => [r.tm, <MoveLink key={`${r.tm}-mv`} name={r.move} stats />, L(r.target), r.location, r.priority, r.desc])}
        />
        <p className="mt-4 mb-2 text-sm font-bold text-ink-faint">선택 기술머신</p>
        <GuideTable
          headers={['TM', '기술', '추천 대상', '입수 위치', '우선순위', '설명']}
          rows={GOLD_STORY_TM_OPTIONAL.map((r) => [r.tm, <MoveLink key={`${r.tm}-mv`} name={r.move} stats />, L(r.target), r.location, r.priority, r.desc])}
        />
